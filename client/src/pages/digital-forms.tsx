...
interface FormTemplate {
  ...
  isPublic: boolean;
  instructorEnabled: boolean | null;
...
function TemplateBuilder({
  ...
  const [instructorEnabled, setInstructorEnabled] = useState<boolean>(editTemplate?.instructorEnabled === true);
        ...
        <div className="space-y-5 py-2">
          ...
          <div className="grid grid-cols-1 gap-4">
            ...
            <div className="flex items-center justify-between rounded-lg border border-slate-200 px-3 py-2.5">
              <div>
                <Label className="flex items-center gap-1.5">Enabled for instructors</Label>
                <p className="text-xs text-slate-400 mt-0.5">
                  Yes = instructors with an access key can see and submit this form from their dashboard. No = hidden from instructors.
                </p>
              </div>
              <Switch checked={instructorEnabled} onCheckedChange={setInstructorEnabled} />
            </div>
          </div>
...
export default function DigitalForms() {
  ...
  const instructorToggleMutation = useMutation({
    mutationFn: ({ id, enabled }: { id: string; enabled: boolean }) =>
      apiRequest("PATCH", `/api/digital-forms/templates/${id}/instructor-access`, { enabled }),
    onSuccess: async (res: any, { enabled }) => {
      queryClient.invalidateQueries({ queryKey: ["/api/digital-forms/templates"] });
      toast({
        title: enabled ? "Enabled for instructors" : "Hidden from instructors",
        description: enabled
          ? "Instructors with an access key can now see and submit this form."
          : "This form is no longer visible in the instructor portal.",
      });
    },
    onError: (e: any) => toast({ title: "Error", description: e.message, variant: "destructive" }),
  });
              ...
              {filteredTemplates.map((template) => {
                ...
                <Card key={template.id} className={`border transition-colors flex flex-col ${
                  ...
                  <CardContent className="pt-0 pb-3 px-4 flex flex-col flex-1">
                    ...
                    <div className="mb-3 flex items-center justify-between gap-2 bg-slate-50 border border-slate-200 rounded-lg px-3 py-2">
                      <span className="text-xs text-slate-600">
                        Enabled for instructors:{" "}
                        <span className={template.instructorEnabled ? "text-green-600 font-semibold" : "text-slate-400 font-semibold"}>
                          {template.instructorEnabled ? "Yes" : "No"}
                        </span>
                      </span>
                      <Switch
                        checked={template.instructorEnabled === true}
                        disabled={instructorToggleMutation.isPending}
                        onCheckedChange={(v) => instructorToggleMutation.mutate({ id: template.id, enabled: v })}
                      />
                    </div>
