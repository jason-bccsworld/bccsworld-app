...
- Module-load schema-init promises must retry on failure: a cold-start DB hiccup once permanently poisoned a lambda instance (every request 503 "storage initializing"). Cache the init promise but reset it on rejection so the next request retries.
- Scanned-PDF OCR verified live (Aug 2026): 3-page image-only scan OCR'd in ~7s on prod; unreadable scans return the friendly 422; no shell-outs involved.
