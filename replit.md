...
- **/admin-dashboard**: Full user management system with 4 tabs — Users (invite, activate/deactivate, reset password, delete, search/filter), Roles & Permissions (permission matrix with per-permission toggles across 8 groups, create/delete custom roles), Organizations, System stats. Backend table: `bccs_role_permissions`. 30 granular permissions across User Management, Compliance Records, Students & Instructors, Documents, Digital Forms, Regulatory & FAA, Audit & Reports, Administration. 4 system roles (admin/auditor/instructor/viewer) seeded with default permissions. Authentication stamps `lastLoginAt`, blocks deactivated users.
